...
__author__ = "Mohit Chandra <mohit@webkul.com>"
__version__ = "0.13.12"
...
class PrestaShopWebServiceError(Exception):
	...
	def __str__(self):
		...
		if self.error_code==401:
			message='Error Code 401- Invalid Prestashop Login Information'
		message=message+repr(self.ps_error_msg)
		return message
...
class PrestaShopWebService(object):
	...
	"""
	MIN_COMPATIBLE_VERSION = '1.4.10'
	MAX_COMPATIBLE_VERSION = '1.7.x.x'
	...
	def __init__(self, api_url, api_key, debug=False, headers=None, client_args=None):
		...
		if self.headers is None:
			self.headers = {'User-agent': 'Prestapi: Python Prestashop Library'}
	...
	def _parse_error(self, xml_content):
		...
		"""
		_logger.info("kkkkkkkkkkkkk---%r",xml_content)
	...
	def _check_status_code(self, status_code, content):
		...
		_logger.info("\\\\\\%r",[status_code,content])
	...
	def _execute(self, url, method, data=None, files=None, add_headers=None):
		...
		_logger.info("-data-------------__>%r",[method,url,type(data)])
		if not files:
			r = self.client.request(method, url, data=data, headers=request_headers)
		else:
			r = self.client.request(method, url, files=files, headers = {'User-agent': 'Prestapi: Python Prestashop Library'})
	...
	def add(self, resource, content, img_filename=None):
		...
		if img_filename:
			...
			if isinstance(img_filename, (str)) and 1<=len(img_filename)<= 255 and "/"	not in img_filename and "\000" not in img_filename and '.' in img_filename:
				if self.debug:
	...
	def add_with_url(self, url, content, img_filename=None):
		...
		if not img_filename:
			...
			_logger.info("-image content----_>%r",[img_filename,content])
			# img_binary = content
	...
	def edit_with_url(self, url, content):
		...
		"""
		headers = {'Content-Type': 'application/x-www-form-urlencoded'}
		_logger.info("iiiiiiiiiiiiiiiiiiiiiiiiii--%r",content)
		r = self._execute(unicode_encode.encode(url), 'PUT', data=content, add_headers=headers)
		return self._parse(r.content)
...
class PrestaShopWebServiceDict(PrestaShopWebService):
	...
	def partial_edit(self, resource, resource_id, fields):
		...
		complete_content.pop('position_in_category',None)
	...
	def add_with_url(self, url, content, img_filename=None):
		...
		if isinstance(res, dict) and res.get('prestashop'):
			...
			# _logger.info("-----res_l2--->%r",type(res_l2))
			res_l2 = list(res_l2)
			if 'content' in res['prestashop'].keys():
				res_l2.remove('content')
			# _logger.info("0000000000000%r",[res,list(res_l2)[0]])
			# res_l2 = list(res_l2)
